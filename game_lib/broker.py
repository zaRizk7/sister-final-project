...
class Broker:
    def __init__(self, name: str = ""):
        ...
        self.inventory = dict(
            Silk=Item(cost=randint(250, 300), amount=randint(3, 5)),
            Silver=Item(cost=randint(450, 600), amount=randint(2, 3)),
            Gold=Item(cost=randint(750, 1000), amount=randint(1, 2)),
            Diamond=Item(cost=randint(1800, 2000), amount=randint(0, 1)),
        )
    ...
    def buy(self, item_name: str, amount: int, total_cost: int) -> str:
        ...
        if buy_success:
            return (
                buy_success,
                f"{self.name} has successfully purchased {item_name} for {total_cost:,.2f}!",
            )
