...
class GameHandler(XMLRPC):
    ...
    def xmlrpc_buy(self, player_name: str,broker_id: int, item_name: str, amount: int, current_cash: int) -> (int, str):
        try:
    ...
    def xmlrpc_sell(self, player_name: str, broker_id: int, item_name: str, amount: int, total_cost: int) -> bool:
        try:
    ...
    def xmlrpc_get_broker_list(self):
        return [f"Broker {b}" for b in self.broker_list]
